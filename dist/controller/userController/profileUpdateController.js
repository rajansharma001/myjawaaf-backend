...
const profileUpdateController = async (req, res) => {
    try {
        ...
        await userSchema_1.User.updateOne({ email }, newUserDetails);
...
const profileImgUpdateController = async (req, res) => {
    try {
        ...
        await userSchema_1.User.updateOne({ email: email }, { $set: { profileImg } });
...
const getUserController = async (req, res) => {
    try {
        const getUser = await userSchema_1.User.find();
        if (!getUser)
